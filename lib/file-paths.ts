...
// Parent directory of `dirPath`, treating the input as a directory (unlike
// `getFileDirectory`, which treats it as a file). The roots — the POSIX root
// `/` and a bare Windows drive `D:` — have no deeper parent and return
// themselves, so listing the "parent" of a missing top-level dir is safe.
export function getParentDirectory(dirPath: string): string {
  const normalized = normalizeFilePathSlashes(dirPath).replace(/\/+$/, "");
