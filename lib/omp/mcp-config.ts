...
export type McpLiveServer = { name: string; source: string; status: McpLiveStatus; type?: string; cwd?: string };
  ...
  return [
    {
      ...
      description: "Native multi-agent orchestration: unified dispatch of 11+ agent CLIs with subtask routing, complexity gating, DAG orchestration, live steering, and a persistent memory bank.",
      author: "Splinterjke",
      version: "v3.0.0",
      tools: [
        "spawn_agent",
      ...
      config: {
        ...
        command: "python3",
        args: [resolvedTarget],
        description: "Agent MCP — unified dispatch and multi-agent orchestration control plane for any agent CLI",
      },
...
export function deleteUserMcpServer(name: string): { path: string } {
  if (!SERVER_NAME.test(name)) throw new Error("Invalid server name");
  const userPath = join(getAgentDir(), "mcp.json");
  return withMcpConfigLock(userPath, () => {
    let currentConfig: McpFile = {};
