...
const { deleteMcpServer, getBuiltinAgentMcpPath, getBuiltinMcpPresets, parseMcpListOutput, readMcpConfig, readUserMcpConfig, redactMcpServer, validateMcpServer, writeMcpServer } = await jiti.import("./mcp-config.ts");
...
test("redacts project MCP credentials and preserves them on edits and renames", () => {
  withWorkspace((cwd) => {
    writeMcpServer(cwd, "private", { type: "stdio", command: "node", env: { API_KEY: "secret" }, headers: { Authorization: "Bearer secret" } });
    const stored = readMcpConfig(cwd).config.mcpServers.private;
    assert.deepEqual(redactMcpServer(stored), { type: "stdio", command: "node" });
    writeMcpServer(cwd, "private", { type: "stdio", command: "node", args: ["server.js"] });
    assert.deepEqual(readMcpConfig(cwd).config.mcpServers.private.env, { API_KEY: "secret" });
    assert.deepEqual(readMcpConfig(cwd).config.mcpServers.private.headers, { Authorization: "Bearer secret" });
    writeMcpServer(cwd, "renamed", { type: "stdio", command: "node", args: ["renamed.js"] }, "private");
    const renamed = readMcpConfig(cwd).config.mcpServers;
...
test("discovers and provides native built-in Agent MCP preset", () => {
  const presets = getBuiltinMcpPresets();
  assert.equal(presets.length > 0, true);
  const agentMcp = presets.find((p) => p.name === "agentmcp");
  assert.ok(agentMcp);
  assert.equal(agentMcp.displayName, "Agent MCP");
  assert.equal(agentMcp.author, "Splinterjke");
  assert.equal(agentMcp.tools.includes("spawn_agent"), true);
  assert.equal(agentMcp.tools.includes("estimate_complexity"), true);
  assert.equal(agentMcp.tools.includes("orchestrate_task"), true);
  assert.equal(agentMcp.config.type, "stdio");
});
