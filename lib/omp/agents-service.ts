...
export function unpackBundled(targetDir: string, force = false): { targetDir: string; total: number; written: number; skipped: number } {
  ...
  const target = wrapWindowsScript(bin, ["agents", "unpack", "--dir", safeTargetDir, "--json", ...(force ? ["--force"] : [])]);
  execFileSync(target.file, target.args, { encoding: "utf8", windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
...
export async function ensureBundledAgentsCache(): Promise<string> {
  ...
  // Force an unpack on every (throttled) check: the bundled set is versioned
  // with the installed omp, so a stale or partially-populated cache (e.g. left
  // over from an older omp) would otherwise under-report the bundled agents.
  // The cache lives in tmp and is cheap to refresh; a missing/failed binary is
  // reported by discovery rather than throwing here.
  try { unpackBundled(cacheDir, true); } catch { /* discovery reports the missing/failed binary */ }
...
export async function discoverAgents(cwd?: string): Promise<{ agents: AgentInfo[]; diagnostics: AgentDiagnostic[]; bundledPath?: string }> {
  ...
  // Return every agent from every scope, keeping same-name copies across
  // scopes distinct. Deduping by name across scopes would let a user or
  // project agent shadow the bundled one, hiding the bundled set from the UI.
  // The UI groups by scope and lets a user scope a save over any copy.
  const agents: AgentInfo[] = [];
  for (const root of roots) {
    agents.push(...(await scanRoot(root, diagnostics)));
  }
  // Scope first (bundled, project, user) then name, so the settings list reads
  // as a stable per-scope grouping.
  const scopeRank = { bundled: 0, project: 1, user: 2 } as const;
  agents.sort((a, b) => scopeRank[a.scope] - scopeRank[b.scope] || a.name.localeCompare(b.name));
  return { agents, diagnostics, bundledPath };
}
