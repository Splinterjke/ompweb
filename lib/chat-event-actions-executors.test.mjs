...
test("http executor substitutes $event-data variables in the body", async () => {
  ...
  const action = create({
    ...
    action: {
      ...
      // Known vars are substituted; the unknown $unknown stays literal.
      body: '{"session":"$session_name","id":"$session_id","reply":"$last_reply","x":"$unknown"}',
    },
  });
  executors.executeChatAction(action, { sessionId: "s1", sessionName: "Demo Session", lastAssistantReply: "The answer is 42" });
  const run = await waitRun(action.id);
  server.close();
  assert.equal(run.ok, true);
  const parsed = JSON.parse(receivedBody);
  assert.equal(parsed.session, "Demo Session");
  assert.equal(parsed.id, "s1");
  assert.equal(parsed.reply, "The answer is 42");
  assert.equal(parsed.x, "$unknown");
});

test("http executor escapes $last_reply properly in a JSON body", async () => {
  let receivedBody = null;
  const server = http.createServer((req, res) => {
    let data = "";
    req.on("data", (c) => (data += c));
  ...
  // The reply carries quotes, newlines, a tab, a backslash and unicode — a
  // raw text substitution would break the JSON body.
  const reply = 'He said "hi"\nsecond line\ta back\\slash and a \u00e9moji \ud83d\ude00';
  const action = create({
    name: "http-json-escape",
    events: ["conversation_completed"],
    action: {
      type: "http",
      method: "POST",
      url: `http://127.0.0.1:${port}/`,
      body: '{"reply":"$last_reply","session":"$session_name"}',
    },
  });
  executors.executeChatAction(action, { sessionId: "s1", sessionName: "Demo", lastAssistantReply: reply });
  const run = await waitRun(action.id);
  server.close();
  assert.equal(run.ok, true);
  const parsed = JSON.parse(receivedBody);
...
test("http executor substitutes raw for non-JSON bodies", async () => {
  let receivedBody = null;
  const server = http.createServer((req, res) => {
    let data = "";
    req.on("data", (c) => (data += c));
  ...
  const action = create({
    ...
    action: {
      ...
      method: "POST",
      url: `http://127.0.0.1:${port}/`,
      body: "reply: $last_reply",
      bodyContentType: "text",
    },
  });
  executors.executeChatAction(action, { sessionId: "s1", sessionName: "Demo", lastAssistantReply: "line1\nline2" });
  const run = await waitRun(action.id);
  server.close();
  assert.equal(run.ok, true);
  assert.equal(receivedBody, "reply: line1\nline2");
});

test("interpolateEventVars substitutes $question (present for question_asked, empty otherwise)", () => {
  assert.equal(
    executors.interpolateEventVars("q=$question", { sessionId: "s1", question: "Which DB: Postgres or SQLite?" }),
    "q=Which DB: Postgres or SQLite?",
  );
  // A payload without a question (any other event) substitutes the empty
  // string, not the literal token — so $question in a body of a non-question
  // event is harmless.
  assert.equal(executors.interpolateEventVars("q=$question", { sessionId: "s1" }), "q=");
});
