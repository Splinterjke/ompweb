...
test("does not treat metadata-only ensure_session files as conversations", () => {
  assert.equal(isUserVisibleSession({ messageCount: 0, title: undefined, firstMessage: "" }), false);
  assert.equal(isUserVisibleSession({ messageCount: 0, title: "(no messages)", firstMessage: "(no messages)" }), false);
  assert.equal(isUserVisibleSession({ messageCount: 0, title: "", firstMessage: "hello" }), true);
  assert.equal(isUserVisibleSession({ messageCount: 0, title: "hello", firstMessage: "(no messages)" }), false);
  assert.equal(isUserVisibleSession({ messageCount: 1, title: undefined, firstMessage: "" }), true);
});
