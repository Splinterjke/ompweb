...
function localRequest(url, init) {
  ...
  // Mirror the Host header from the request URL so the proxy's
  // isApiRequestHostAllowed DNS-rebinding guard (loopback allowed) sees a
  // well-formed host instead of an empty one.
...
test("redirects browser requests to the password screen and blocks unauthenticated APIs", () => {
  ...
  try {
    const pageResponse = proxy(localRequest("http://localhost:30177/"));
    assert.equal(pageResponse.status, 307);
    assert.equal(pageResponse.headers.get("location"), "http://localhost:30177/login");

    const crossSitePageResponse = proxy(localRequest("http://localhost:30177/", {
      headers: { "sec-fetch-site": "cross-site" },
    }));
    assert.equal(crossSitePageResponse.status, 307);

    const apiResponse = proxy(localRequest("http://localhost:30177/api/sessions"));
    assert.equal(apiResponse.status, 401);

    const session = createWebSession("secret");
    const signedInResponse = proxy(localRequest("http://localhost:30177/", {
