...
test("slash token extraction follows the caret, not the input start", async () => {
  const { extractSlashQuery } = await loadSubject();

  // Line start and bare slash.
  assert.deepEqual(extractSlashQuery(""), null);
  assert.deepEqual(extractSlashQuery("/"), { start: 0, query: "" });
  assert.deepEqual(extractSlashQuery("/plan"), { start: 0, query: "plan" });

  // Slash typed after real text — the regression: the palette must open here.
  assert.deepEqual(extractSlashQuery("帮我看看 /pl"), { start: 5, query: "pl" });
  // CJK text directly before the slash also triggers (no whitespace needed).
  assert.deepEqual(extractSlashQuery("帮我看看/pl"), { start: 4, query: "pl" });
  assert.deepEqual(extractSlashQuery("，/foo"), { start: 1, query: "foo" });

  // Paths and URLs must never trigger.
  assert.equal(extractSlashQuery("a/b"), null);
  assert.equal(extractSlashQuery("src/lib/a/b"), null);
  assert.equal(extractSlashQuery("http://x"), null);
  assert.equal(extractSlashQuery("v1.2/3"), null);

  // Whitespace after the token (command args) closes the menu.
  assert.equal(extractSlashQuery("/plan "), null);
  assert.equal(extractSlashQuery("帮我看看 /plan arg"), null);
});
