...
test("only existing session-discovered paths become automatic workspaces", (t) => {
  const root = mkdtempSync(join(tmpdir(), "omp-web-discovered-projects-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const live = join(root, "live");
  mkdirSync(live);
  const missing = join(root, "removed-worktree");
  assert.deepEqual(existingDiscoveredProjectPaths([live, missing, live]), [live]);
});
...
test("worktree paths canonicalize to the main repo root when registered", async (t) => {
  ...
  try {
    ...
    // macOS /var is a symlink to /private/var — compare resolved forms so the
    // assertion is immune to which spelling git/node report.
    const repoReal = realpathSync(repo);
    assert.equal(realpathSync(project.projectRoot), repoReal);
    ...
    assert.equal(realpathSync(registered.projects[0].path), repoReal);
    ...
    assert.deepEqual(merged.map((p) => realpathSync(p.path)), [repoReal]);
  } finally {
...
test("resolveProject returns canonical on-disk casing for plain directories", (t) => {
  ...
  return resolveProject(dir).then((info) => {
    ...
    assert.equal(realpathSync(info.projectRoot), realpathSync(real));
  });
});
