...
test("git routes: a non-repo is a normal situation, never recorded as a backend error", async () => {
  // isNonRepositoryError recognizes both the Rust host's structured code and
  // the Node fallback's plain message, so no code path records it.
  const helper = await readFile(new URL("./git-nonrepo.ts", import.meta.url), "utf8");
  assert.match(helper, /not_a_git_repository/);
  assert.match(helper, /Not a Git repository/);
  const diff = await readFile(new URL("../app/api/git/diff/route.ts", import.meta.url), "utf8");
  // Both diff modes answer gracefully for a non-repo instead of recording.
  assert.match(diff, /isNonRepositoryError\(error\)\) \{\s*\n\s*return NextResponse\.json\(\{ supported: false \}\)/);
  assert.doesNotMatch(diff, /isNonRepositoryError\(error\)\) \{\s*\n\s*recordBackendError/);
  // The other git routes keep their error response but skip the record.
  for (const [rel, kind] of [
    ["status", "git_status_failed"],
    ["branches", "git_branches_failed"],
    ["checkout", "git_checkout_failed"],
    ["commit", "git_commit_failed"],
    ["push", "git_push_failed"],
  ]) {
    const source = await readFile(new URL(`../app/api/git/${rel}/route.ts`, import.meta.url), "utf8");
    assert.match(source, /if \(!isNonRepositoryError\(error\)\) \{\s*\n\s*recordBackendError\("/);
