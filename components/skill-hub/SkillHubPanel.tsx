        ...
        <form className={css.form} onSubmit={(event) => { void create(event) }}>
          ...
          <div className={css.formRow}>
            ...
            <select className={css.select} value={formRoot} onChange={(event) => { setFormRoot(event.target.value as WritableRoot) }}>
              <option value="user-agents">{tt('root.user-agents')}</option>
              <option value="user-omp">{tt('root.user-omp')}</option>
              <option value="user-dsh">{tt('root.user-dsh')}</option>
              <option value="user-codex">{tt('root.user-codex')}</option>
              {workspace ? (
                <>
                  <option value="project-agents">{tt('root.project-agents')}</option>
                  <option value="project-omp">{tt('root.project-omp')}</option>
                </>
              ) : null}
            </select>
          </div>
      ...
      {tab === 'market' ? (
        ...
        <>
          <div className={css.filterBar}>
              ...
              <>
                ...
                <select className={css.select} value={sourceFilter} onChange={(event) => { setSourceFilter(event.target.value) }}>
                  ...
                  {sourceOptions.map((source) => {
                    const label = source === PRIVATE_SOURCE ? tt('filter.private') : source === 'user-codex' ? tt('src.user-codex') : source === 'user-agents' ? tt('src.user-agents') : source === 'user-omp' ? tt('src.user-omp') : source === 'user-dsh' ? tt('src.user-dsh') : source;
            ...
            <button type="button" className={css.button + (editMode ? ' ' + css.primary : '')} style={{ marginLeft: 'auto' }} onClick={() => setEditMode((v) => !v)}>{editMode ? tt('panel.done') : tt('panel.edit')}</button>
          </div>
