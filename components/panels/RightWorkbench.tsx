...
function TemporarySideChat({ cwd }: { cwd: string | null }) {
  ...
  if (!cwd) return <div style={{ height: "100%", display: "grid", placeItems: "center", padding: 20, color: "var(--text-dim)", fontSize: 12, textAlign: "center" }}>Select a workspace first, then start a temporary OMP conversation.</div>;
  return <div style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column" }}><div style={{ padding: "6px 10px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)", color: "var(--text-dim)", fontSize: 10.5 }}>Temporary OMP · a separate session that does not interrupt the main conversation</div><div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: 9, display: "grid", alignContent: "start", gap: 7 }}>{messages.length === 0 && !running && <div style={{ color: "var(--text-dim)", fontSize: 11, textAlign: "center", padding: 16 }}>Type a question to start a temporary OMP.</div>}{messages.map((message, index) => <div key={index} style={{ padding: "6px 8px", borderRadius: 7, background: message.role === "user" ? "var(--user-bg)" : "var(--bg-panel)", color: "var(--text)", fontSize: 11.5, lineHeight: 1.5 }}>{Array.isArray(message.content) ? (message.content as Array<{ type?: string; text?: string; toolName?: string; input?: unknown }>).map((block, blockIndex) => block.type === "toolCall" ? <pre key={blockIndex} style={{ margin: 0, whiteSpace: "pre-wrap", color: "var(--accent)", font: "10.5px/1.45 var(--font-mono)" }}>→ {block.toolName ?? "tool"} {JSON.stringify(block.input ?? {})}</pre> : <span key={blockIndex}>{block.text ?? ""}</span>) : String(message.content ?? "")}</div>)}{running && <div style={{ color: "var(--text-dim)", fontSize: 11 }}>Temporary OMP is working…</div>}{error && <div role="alert" style={{ color: "var(--status-error)", fontSize: 11 }}>{error}</div>}</div><form onSubmit={(event) => { event.preventDefault(); void send(); }} style={{ display: "flex", gap: 6, padding: 7, borderTop: "1px solid var(--border)", background: "var(--bg-panel)" }}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask the temporary OMP…" aria-label="Ask the temporary OMP" style={{ minWidth: 0, flex: 1, height: 28, padding: "0 8px", border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg)", color: "var(--text)", outline: 0, fontSize: 11.5 }} /><button type="submit" disabled={!input.trim() || running} title="Send to the temporary OMP" aria-label="Send to the temporary OMP" style={{ width: 44, border: 0, borderRadius: 6, background: "var(--accent)", color: "var(--bg)", cursor: input.trim() && !running ? "pointer" : "not-allowed", opacity: input.trim() && !running ? 1 : 0.5, fontSize: 11 }}>Send</button></form></div>;
}
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", minHeight: 0, background: "var(--bg)" }}>
      <form onSubmit={(event) => { event.preventDefault(); submit(); }} style={{ display: "flex", gap: 6, padding: "7px 8px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)" }}>
        ...
        <input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={t("rightWorkbench.browserPlaceholder") === "rightWorkbench.browserPlaceholder" ? "Enter an http(s) URL…" : t("rightWorkbench.browserPlaceholder")} aria-label={t("rightWorkbench.browserPlaceholder")} style={{ minWidth: 0, flex: 1, border: 0, outline: 0, background: "transparent", color: "var(--text)", fontSize: 11.5 }} />
        <button type="submit" disabled={!/^https?:\/\//i.test(draft.trim())} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "var(--bg)", color: "var(--text)", padding: "3px 8px", cursor: "pointer", fontSize: 11 }}>{t("rightWorkbench.browserGo") === "rightWorkbench.browserGo" ? "Open" : t("rightWorkbench.browserGo")}</button>
      </form>
      {loading ? (
        ...
        <div role="alert" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, color: "var(--status-error)", fontSize: 12, textAlign: "center", padding: 20 }}><span>{error}</span><button type="button" onClick={() => void submit()} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "var(--bg-panel)", color: "var(--text)", padding: "5px 9px", cursor: "pointer", fontSize: 11 }}>Retry</button></div>
      ) : (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-dim)", fontSize: 12, textAlign: "center", padding: 20 }}>{t("rightWorkbench.browserHint") === "rightWorkbench.browserHint" ? "Enter a URL to preview it in the workspace" : t("rightWorkbench.browserHint")}</div>
      )}
    </div>
  );
}
...
export function RightWorkbench({
  ...
  const renderView = useCallback((view: WorkbenchView) => {
    ...
    if (view === "git") return cwd && onOpenFile ? <GitChangesPanel cwd={cwd} onOpenFile={onOpenFile} /> : <div style={{ height: "100%", display: "grid", placeItems: "center", padding: 20, color: "var(--text-dim)", fontSize: 12, textAlign: "center" }}>Select a workspace first.</div>;
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      {layout.panes.map((pane, index) => <div key={pane.id} style={{ position: "relative", minWidth: 0, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column", flex: index === 0 && layout.panes.length > 1 ? `0 0 ${layout.split}%` : 1 }}>
        <div style={{ flex: 1, minHeight: 0, overflow: "hidden", position: "relative" }}>
          ...
          {!pane.active && <div style={{ height: "100%", display: "grid", placeItems: "center", color: "var(--text-dim)", fontSize: 11, padding: 16, textAlign: "center" }}>Click the + above to add a workspace</div>}
        </div>
