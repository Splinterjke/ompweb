...
function TemporarySideChat({ cwd }: { cwd: string | null }) {
  ...
  if (!cwd) return <div style={{ height: "100%", display: "grid", placeItems: "center", padding: 20, color: "var(--text-dim)", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", textAlign: "center" }}>Select a workspace first, then start a temporary OMP conversation.</div>;
  return <div style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column" }}><div style={{ padding: "6px 10px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)", color: "var(--text-dim)", fontSize: "calc(10.5px * var(--ui-font-scale-sm, 1))" }}>Temporary OMP · a separate session that does not interrupt the main conversation</div><div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: 9, display: "grid", alignContent: "start", gap: 7 }}>{messages.length === 0 && !running && <div style={{ color: "var(--text-dim)", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", textAlign: "center", padding: 16 }}>Type a question to start a temporary OMP.</div>}{messages.map((message, index) => <div key={index} style={{ padding: "6px 8px", borderRadius: 7, background: message.role === "user" ? "var(--user-bg)" : "var(--bg-panel)", color: "var(--text)", fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", lineHeight: 1.5 }}>{Array.isArray(message.content) ? (message.content as Array<{ type?: string; text?: string; toolName?: string; input?: unknown }>).map((block, blockIndex) => block.type === "toolCall" ? <pre key={blockIndex} style={{ margin: 0, whiteSpace: "pre-wrap", color: "var(--accent)", font: "10.5px/1.45 var(--font-mono)" }}>→ {block.toolName ?? "tool"} {JSON.stringify(block.input ?? {})}</pre> : <span key={blockIndex}>{block.text ?? ""}</span>) : String(message.content ?? "")}</div>)}{running && <div style={{ color: "var(--text-dim)", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>Temporary OMP is working…</div>}{error && <div role="alert" style={{ color: "var(--status-error)", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>{error}</div>}</div><form onSubmit={(event) => { event.preventDefault(); void send(); }} style={{ display: "flex", gap: 6, padding: 7, borderTop: "1px solid var(--border)", background: "var(--bg-panel)" }}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask the temporary OMP…" aria-label="Ask the temporary OMP" style={{ minWidth: 0, flex: 1, height: 28, padding: "0 8px", border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg)", color: "var(--text)", outline: 0, fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))" }} /><button type="submit" disabled={!input.trim() || running} title="Send to the temporary OMP" aria-label="Send to the temporary OMP" style={{ width: 44, border: 0, borderRadius: 6, background: "var(--accent)", color: "var(--bg)", cursor: input.trim() && !running ? "pointer" : "not-allowed", opacity: input.trim() && !running ? 1 : 0.5, fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>Send</button></form></div>;
}
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", minHeight: 0, background: "var(--bg)" }}>
      ...
      {loading ? (
        ...
        <div style={{ flex: 1, minHeight: 0, position: "relative" }}>
          ...
          <button type="button" onClick={() => window.open(url, "_blank", "noopener,noreferrer")} title={t("rightWorkbench.browserExternal") === "rightWorkbench.browserExternal" ? "Open in system browser" : t("rightWorkbench.browserExternal")} style={{ position: "absolute", top: 8, right: 8, display: "inline-flex", gap: 4, alignItems: "center", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "color-mix(in srgb, var(--bg-panel) 90%, transparent)", color: "var(--text-muted)", padding: "4px 7px", cursor: "pointer", fontSize: "calc(10.5px * var(--ui-font-scale-sm, 1))" }}><ExternalLink size={12} aria-hidden="true" />{t("rightWorkbench.browserExternal") === "rightWorkbench.browserExternal" ? "Open externally" : t("rightWorkbench.browserExternal")}</button>
        </div>
      ) : error ? (
