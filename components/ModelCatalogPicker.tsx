...
export function ModelCatalogPicker({ open, providerName, providerBaseUrl, existingIds, onAdd, onClose }: ModelCatalogPickerProps) {
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      ...
      >
        <DialogTitle style={{ margin: "14px 18px 8px", fontSize: "calc(18px * var(--ui-font-scale-lg, 1))" }}>{t("modelsConfig.catalogTitle")}</DialogTitle>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 14px 14px" }}>
          {error ? (
            <div role="alert" style={{ padding: "20px 0", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", color: "var(--status-error)", textAlign: "center" }}>
              {t("modelsConfig.catalogError", { error })}
            </div>
          ) : loading ? (
            <div role="status" aria-live="polite" style={{ padding: "20px 0", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", color: "var(--text-dim)", textAlign: "center" }}>{t("modelsConfig.catalogLoading")}</div>
          ) : results === null ? (
            <div style={{ padding: "20px 0", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", color: "var(--text-dim)", textAlign: "center" }}>{t("modelsConfig.catalogSearchPlaceholder")}</div>
          ) : results.length === 0 ? (
            <div style={{ padding: "20px 0", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", color: "var(--text-dim)", textAlign: "center" }}>{t("modelsConfig.catalogNoResults")}</div>
          ) : (
            results.map((entry) => {
                ...
                <div key={entry.key} style={rowStyle}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                      <span style={{ fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", fontFamily: "var(--font-mono)", color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.id}</span>
                      {entry.reasoning && (
                        <span style={{ fontSize: "calc(9px * var(--ui-font-scale-sm, 1))", padding: "1px 4px", background: "color-mix(in srgb, var(--accent) 12%, transparent)", color: "var(--accent)", borderRadius: 3, flexShrink: 0 }}>T</span>
                      )}
                    </div>
                    <div style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {entry.providerName}
                  ...
                  {alreadyAdded ? (
                    <span style={{ flexShrink: 0, padding: "5px 12px", background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text-dim)", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))" }}>
                    ...
                    <button
                      ...
                      style={{ flexShrink: 0, padding: "5px 12px", background: "var(--accent)", border: "none", borderRadius: "var(--radius-control)", color: "var(--on-accent)", cursor: "pointer", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", fontWeight: 600 }}
                    >
