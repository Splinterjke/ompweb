...
function DiffLines({ diff }: { diff: string }) {
  ...
  return (
    <pre style={{ margin: 0, padding: "8px 0", overflow: "auto", fontFamily: "var(--font-mono)", fontSize: 11, lineHeight: 1.5, border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg)" }}>
      {lines.map((line, i) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <DialogTitle style={{ fontSize: 15, margin: 0, flex: 1 }}>{t("gitGraph.title")}</DialogTitle>
        ...
        {!cwd ? (
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: "var(--text-dim)", fontSize: 13 }}>
            <LoaderCircle size={14} strokeWidth={1.8} style={{ animation: "spin 1s linear infinite" }} aria-hidden="true" />
            {t("gitGraph.loading")}
          </div>
        ) : error ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, color: "var(--status-error)", fontSize: 13, overflowWrap: "anywhere" }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 7, flex: 1, minWidth: 0, padding: "0 10px 0 6px" }}>
                      ...
                      {commit.isMerge && (
                        <span style={{ flexShrink: 0, fontSize: 10, color: "var(--text-dim)" }} title={t("gitGraph.merge")}>
                          ⑂
                        </span>
                      )}
                      <span style={{ flexShrink: 0, fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-dim)" }} title={commit.hash}>
                        {commit.shortHash}
                      </span>
                      <span style={{ flex: 1, minWidth: 0, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--text)" }}>
                        {commit.subject}
                      </span>
                      <span style={{ flexShrink: 0, fontSize: 11, color: "var(--text-muted)", maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {commit.author}
                      </span>
                    </div>
                  </div>
              ...
              >
                <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "12px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, overflowWrap: "anywhere", lineHeight: 1.4 }}>{t("gitGraph.commit")}: {selected.subject}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                      <code style={{ fontSize: 11, color: "var(--text-muted)", whiteSpace: "nowrap", overflowX: "auto", flex: 1, minWidth: 0, scrollbarWidth: "thin" }}>{selected.hash}</code>
                      <button
                        type="button"
                        ...
                        className="ui-focus-ring"
                        style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 6px", border: "1px solid var(--border)", borderRadius: 4, background: "none", color: copied ? "var(--accent)" : "var(--text-dim)", cursor: "pointer", fontSize: 10, flexShrink: 0 }}
                      >
                ...
                <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", display: "grid", gridTemplateColumns: "auto 1fr", gap: "5px 12px", fontSize: 11, flexShrink: 0 }}>
                ...
                {selected.body && (
                  <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0, maxHeight: "30%", overflow: "auto" }}>
                    <pre style={{ margin: 0, fontSize: 12, lineHeight: 1.55, color: "var(--text)", whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>{selected.body}</pre>
                  </div>
                )}
                <div style={{ padding: "10px 14px 4px", fontSize: 11, fontWeight: 650, color: "var(--text-dim)", flexShrink: 0 }}>
                  {t("gitGraph.files")} ({selected.files.length})
                </div>
                <div style={{ flex: 1, overflow: "auto", padding: "4px 10px 14px" }}>
                  {selected.files.length === 0 && <div style={{ padding: "8px 4px", fontSize: 11, color: "var(--text-dim)" }}>{t("gitGraph.noDiff")}</div>}
                  {selected.files.map((file) => (
                    ...
                    >
                      <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "var(--font-mono)", fontSize: 11 }}>
                        {file.path}
                      </span>
                      {file.additions !== null && <span style={{ flexShrink: 0, fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--status-success)" }}>+{file.additions}</span>}
                      {file.deletions !== null && <span style={{ flexShrink: 0, fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--status-error)" }}>-{file.deletions}</span>}
                      {file.additions === null && file.deletions === null && <span style={{ flexShrink: 0, fontSize: 10, color: "var(--text-dim)" }}>{t("gitGraph.binaryFile")}</span>}
                    </button>
                  ))}
                </div>
              </aside>
            ...
            {selected && (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0, minWidth: 0 }}>
                  <span style={{ flex: 1, minWidth: 0, fontSize: 11, fontFamily: "var(--font-mono)", color: diff ? "var(--text-muted)" : "var(--text-dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                ...
                <div style={{ flex: 1, overflow: "auto", padding: "8px 10px" }}>
                  {diffError && <div style={{ padding: "8px 6px", fontSize: 11, color: "var(--status-error)", overflowWrap: "anywhere" }}>{t("gitGraph.diffError", { error: diffError })}</div>}
                  {diffLoading && !diffError && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 6px", fontSize: 11, color: "var(--text-dim)" }}>
                      {t("gitGraph.diffLoading")}
                    </div>
                  )}
                  {diff && !diffLoading && !diffError && (
                    diff.binary ? (
                      <div style={{ fontSize: 11, color: "var(--text-dim)", padding: "6px 0" }}>{t("gitGraph.binaryFile")}</div>
                    ) : (
