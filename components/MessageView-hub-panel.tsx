...
export function HubResultPanel({ input, result }: { input: unknown; result?: ToolResultMessage }) {
  ...
  if (input.op === "send") {
    ...
    return (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-muted)" }}>
          <span style={{ fontWeight: 600, color: "var(--text)" }}>
            {`IRC → ${send.to.join(", ")}`}
          </span>
          {receipts.length > 0 && (
            <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", color: "var(--text-dim)", fontSize: "calc(10.5px * var(--ui-font-scale-sm, 1))" }}>
      ...
      >
        ...
        {jobs.map((job) => {
          ...
          return (
            <div
              key={job.id}
              style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0, fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))" }}
            >
              ...
              {duration && (
                <span style={{ flexShrink: 0, fontFamily: "var(--font-mono)", fontSize: "calc(10px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)" }}>
                  {duration}
                </span>
              )}
            </div>
