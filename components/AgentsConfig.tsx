...
export function AgentsConfig({ cwd }: { cwd: string | null }) {
  ...
  const [selected, setSelected] = useState<string | null>(null); // "scope:filename"
  ...
  const load = useCallback(async (forceSelect = false) => {
    ...
    try {
      ...
      if (forceSelect || !creating) {
        const currentKey = selectedRef.current;
        const chosen = (currentKey ? list.find((a) => agentKey(a) === currentKey) : undefined) ?? list[0] ?? null;
        selectedRef.current = chosen ? agentKey(chosen) : null;
        setSelected(chosen ? agentKey(chosen) : null);
  ...
  const agentKey = (a: AgentInfo) => `${a.scope}:${fileStem(a.filePath)}`;
  // Group the (already scope-sorted) list into per-scope sections so the
  // settings view shows bundled, project, and user agents as distinct groups.
  const grouped = useMemo(() => {
    const order: Array<"bundled" | "project" | "user"> = ["bundled", "project", "user"];
    return order
      .map((scope) => ({ scope, items: filtered.filter((a) => a.scope === scope) }))
      .filter((g) => g.items.length > 0);
  }, [filtered]);
  const active = useMemo(() => (creating || !selected ? null : agents.find((a) => agentKey(a) === selected) ?? null), [agents, selected, creating]);
  ...
  const pick = (a: AgentInfo) => { selectedRef.current = agentKey(a); setCreating(false); setSelected(agentKey(a)); fillForm(a); setMessage(null); };
  ...
  const cancelCreate = () => {
    ...
    if (agents[0]) { selectedRef.current = agentKey(agents[0]); setSelected(agentKey(agents[0])); fillForm(agents[0]); }
  ...
  const save = async () => {
    ...
    try {
      ...
      selectedRef.current = `${scope}:${trimmedName}`; setCreating(false); setSelected(`${scope}:${trimmedName}`); await load(true);
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", overflow: "hidden", display: "flex", flexDirection: "column", minHeight: 0 }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {grouped.length === 0 ? (
              ...
              grouped.map((group) => (
                <div key={group.scope}>
                  <div style={{ padding: "5px 10px", fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-dim)", borderBottom: "1px solid var(--border)", background: "var(--bg-subtle)" }}>
                    {t(`agentsConfig.scope${group.scope === "bundled" ? "Bundled" : group.scope === "project" ? "Project" : "User"}`)} · {group.items.length}
                  </div>
                  {group.items.map((a) => {
                    const isSelected = !creating && selected === agentKey(a);
                    const dot = !a.valid ? "var(--status-error, #e5484d)" : a.enabled ? "var(--accent)" : "var(--border)";
                    return (
                      <button key={agentKey(a)} type="button" onClick={() => pick(a)} style={{ width: "100%", textAlign: "left", display: "flex", gap: 8, padding: "9px 10px", border: "none", borderLeft: isSelected ? "2px solid var(--accent)" : "2px solid transparent", background: isSelected ? "var(--bg-selected)" : "transparent", cursor: "pointer", alignItems: "flex-start" }}>
