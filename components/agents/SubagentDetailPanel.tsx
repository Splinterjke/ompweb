...
export function SubagentDetailPanel({ subagent, sessionId, onBack }: { subagent: SubagentInfo; sessionId: string | null; onBack: () => void }) {
  ...
  return <div style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column", background: "var(--bg)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 7, minHeight: 34, padding: "0 9px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)" }}><button type="button" onClick={onBack} aria-label="Back to agents" title="Back to agents" style={{ display: "grid", placeItems: "center", width: 24, height: 24, border: 0, background: "transparent", color: "var(--text-muted)", cursor: "pointer" }}><ArrowLeft size={14} aria-hidden="true" /></button><Bot size={14} style={{ color: "var(--accent)" }} aria-hidden="true" /><strong style={{ fontFamily: "var(--font-mono)", fontSize: 11.5, color: "var(--accent)" }}>{subagent.agent}</strong><span style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--text-muted)", fontSize: 11 }}>{subagent.task ?? subagent.description ?? ""}</span></div>
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: 10, display: "grid", alignContent: "start", gap: 8 }}>
      {loading && <div role="status" style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--text-dim)", fontSize: 11 }}><LoaderCircle size={13} className="icon-spin" aria-hidden="true" />Loading subagent messages…</div>}
      ...
      {!loading && !error && messages.length === 0 && <div style={{ color: "var(--text-dim)", fontSize: 11, textAlign: "center", padding: 20 }}>No messages to read yet.</div>}
      {(() => {
