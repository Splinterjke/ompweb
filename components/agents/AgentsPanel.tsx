  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        ...
        <span style={{ fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", fontWeight: 600, color: "var(--text)" }}>{t("chatWindow.subagentsPanel")}</span>
        <span
          style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: "calc(10.5px * var(--ui-font-scale-sm, 1))", color: counts.running > 0 ? "var(--accent)" : "var(--text-dim)" }}
      ...
      <div role="toolbar" aria-label={t("chatWindow.subagentsFilter") ?? "Filter agents"} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 8px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)", flexShrink: 0 }}>
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("chatWindow.subagentsSearch") ?? "Filter agents…"} aria-label={t("chatWindow.subagentsSearch") ?? "Filter agents"} style={{ minWidth: 0, flex: 1, border: 0, outline: 0, background: "transparent", color: "var(--text)", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }} />
      ...
      >
        ...
        {liveSorted.length === 0 && historySorted.length === 0 && (
          <div style={{ padding: "24px 14px", color: "var(--text-dim)", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", textAlign: "center" }}>
          ...
          <>
            <div
              ...
              style={{
                ...
                fontSize: "calc(10px * var(--ui-font-scale-sm, 1))", fontWeight: 700, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.04em",
              }}
            >
              {t("chatWindow.historySubagents") ?? "History"}
              <span style={{ fontFamily: "var(--font-mono)", fontWeight: 400, fontSize: "calc(9.5px * var(--ui-font-scale-sm, 1))" }}>{historySorted.length}</span>
            </div>
