...
interface Props {
  ...
  /** Fired when the rebuild reaper verifies the fresh bundle is served and
   *  broadcasts a UI refresh over SSE — the authoritative "this boot is a
   *  rebuild" signal. The server_boot path misses it (it reads the flag before
...
/** Persisted pixel height of the resizable File Explorer section. */
const EXPLORER_HEIGHT_KEY = "omp-web:explorer-section-height";
...
export function SessionSidebar({ selectedSessionId, optimisticSession, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onWorkspaceOptionsChange, addProjectOpen, setAddProjectOpen, onOpenFile, explorerRefreshKey, onExplorerRefresh, explorerRefreshing, onExplorerRefreshDone, onAtMention, onAtMentions, onOpenSettings, onOpenRemote, onOpenArchive, onServerRestarted, onUiUpdated, onChatEventAction, onOpenGitGraph, updateAvailable, settingsOpen }: Props) {
  ...
  const [explorerOpen, setExplorerOpen] = useState<boolean>(() => {
    ...
    try {
      return window.localStorage.getItem("omp-web:explorer-section-open") === "1";
    } catch {
      return false;
    }
  });
  ...
  const [schedOpen, setSchedOpen] = useState<boolean>(() => {
    if (typeof window === "undefined") return false;
    try {
      return window.localStorage.getItem("omp-web:schedulers-panel-open") === "1";
    } catch {
      return false;
    }
  });
  const [schedHeight, setSchedHeight] = useState<number>(() => readStoredSectionHeight("omp-web:schedulers-section-height"));
  const [usageOpen, setUsageOpen] = useState<boolean>(() => {
    if (typeof window === "undefined") return false;
    try {
      return window.localStorage.getItem("omp-web:usage-panel-open") === "1";
    } catch {
      return false;
    }
  });
  ...
  const setSchedOpenPersist = useCallback((next: boolean) => {
    setSchedOpen(next);
    try {
      window.localStorage.setItem("omp-web:schedulers-panel-open", next ? "1" : "0");
    } catch {
      /* ignore */
    }
  }, []);
  const setUsageOpenPersist = useCallback((next: boolean) => {
    setUsageOpen(next);
    try {
      window.localStorage.setItem("omp-web:usage-panel-open", next ? "1" : "0");
    } catch {
      /* ignore */
    }
  }, []);
  ...
  // Workspaces section collapse (chevron in the header toggles the list).
  const [workspacesOpen, setWorkspacesOpen] = useState<boolean>(() => {
    if (typeof window === "undefined") return true;
    try {
      return window.localStorage.getItem("omp-web:workspaces-open") !== "0";
    } catch {
      return true;
    }
  });
  const toggleWorkspaces = useCallback(() => {
    setWorkspacesOpen((next) => {
      const value = !next;
      try {
        window.localStorage.setItem("omp-web:workspaces-open", value ? "1" : "0");
      } catch {
        /* ignore */
      }
      return value;
  ...
  // ---- Adjacent-transfer resize -----------------------------------------
  // Each resizable section (Explorer / Schedulers / Usage) borrows from the
  // nearest OPEN section above it (or the flex Workspaces list when nothing
  // above is open). A transfer conserves the column's total height, so the
  // bottom sections (Usage / Settings) never scroll off-screen and dragging
  // up stops when the section touches the one above (partner clamped to its
  // header + SECTION_MIN_CONTENT). The list is a flex item, so it absorbs
  // changes with no explicit set.
  const explorerRendered = Boolean(selectedCwdProp || selectedCwd) && explorerOpen;
  // The "flex partner" is the free-space item above a section: the Workspaces
  // list when it's open (sits above Explorer). When Workspaces is collapsed the
  // only flex item is the spacer, which sits BELOW the sections and cannot
  // shrink to give a section room — so no section can grow in that layout
  // (the column already fits; there's no overflow to relieve). Both are
  // `flex: 1 1 0`, so they absorb a transfer with no explicit set.
  const flexPartner = {
    getHeight: () => (workspacesOpen ? listRef.current?.getBoundingClientRect().height ?? 0 : 0),
    getMin: () => (workspacesOpen ? 80 : 0),
    set: () => {},
  };
  const explorerSectionPartner = {
    getHeight: () => explorerHeight,
    getMin: () => {
      const h = explorerHeaderRef.current?.getBoundingClientRect().height;
      return h ? Math.ceil(h) + SECTION_MIN_CONTENT : SECTION_MIN_HEIGHT;
    },
    set: (v: number) => setExplorerHeight(v),
  };
  const schedSectionPartner = {
    getHeight: () => schedHeight,
    getMin: () => {
      const h = schedHeaderRef.current?.getBoundingClientRect().height;
      return h ? Math.ceil(h) + SECTION_MIN_CONTENT : SECTION_MIN_HEIGHT;
    },
  ...
  // Boot epoch (server start time) of the instance our SSE stream is
  // connected to. The first `server_boot` frame stores it; a later frame with a
  // different epoch means the server restarted under us (rebuild/restart) — the
  // bundle this page loaded is stale, so surface the "OmpWeb started" notice
  // (with a Refresh button) instead of reloading automatically.
  const serverBootRef = useRef<number | null>(null);
    ...
    source.onmessage = (e) => {
      try {
        ...
        if (data.type === "server_boot") {
          ...
          // from the epoch we first saw, the server restarted while we were
          // connected — the bundle this page loaded is stale. Rather than
          // reload automatically, surface the "OmpWeb started" notice so the
          // user can choose to refresh.
          if (typeof data.startedAt === "number") {
            if (serverBootRef.current === null) {
              serverBootRef.current = data.startedAt;
            } else if (serverBootRef.current !== data.startedAt) {
              onServerRestarted?.();
            }
          }
        } else if (data.type === "refresh") {
          // Rebuild reaper verified the fresh bundle and broadcast a refresh.
          // Forward it so the parent can upgrade the notice to "updated".
          if (data.updated) onUiUpdated?.();
  ...
  const handleNewSessionInProject = useCallback((projectPath: string) => {
    const tempId = typeof crypto.randomUUID === "function"
      ? crypto.randomUUID()
      : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;
    setSelectedCwd(projectPath);
    onNewSession?.(tempId, projectPath);
  }, [onNewSession]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflowY: "auto", overflowX: "hidden" }}>
      ...
      <div className="sidebar-section-header" style={{ flexShrink: 0, padding: "4px 6px 2px 8px", display: "flex", alignItems: "center", gap: 2, transition: "background var(--dur-fast) var(--ease-out-warm)" }}>
        <button
          type="button"
        ...
        >
          <PanelsTopLeft
            ...
            aria-hidden="true"
          />
          <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t("projects.heading")}</span>
        </button>
        <SidebarIconButton
        ...
        <SectionChevron open={workspacesOpen} />
      </div>
      {searchOpen && (
      ...
      {(selectedCwdProp || selectedCwd) && (
        <div
          style={{
            ...
            flexDirection: "column",
            flexShrink: 0,
            height: explorerOpen ? explorerHeight : undefined,
            minHeight: 0,
            overflow: "hidden",
            transition: anyDragging ? "none" : "height var(--dur-med) var(--ease-out-warm)",
          }}
        >
          {explorerOpen && (
            <SeparatorHandle
              label={t("sessionSidebar.explorer")}
              ariaLabel={t("sessionSidebar.resizeSection", { name: t("sessionSidebar.explorer") })}
              dragging={explorerResize.dragging}
              onMouseDown={explorerResize.start}
              onDoubleClick={explorerResize.reset}
              onKeyDown={explorerResize.handleKeyDown}
            />
          )}
          <div ref={explorerHeaderRef} className="sidebar-section-header" style={{ display: "flex", alignItems: "center", flexShrink: 0, paddingRight: 6, transition: "background var(--dur-fast) var(--ease-out-warm)" }}>
            <button
              onClick={() => setExplorerOpen((v) => {
                const next = !v;
                try {
                  window.localStorage.setItem("omp-web:explorer-section-open", next ? "1" : "0");
                } catch {
                  /* ignore */
                }
                return next;
            ...
            <div
              ...
              style={{
                display: explorerOpen ? "flex" : "none",
                alignItems: "center",
              }}
            >
            ...
            <SectionChevron open={explorerOpen} />
          </div>
          <div
      ...
      <UsageSidebarPanel
        open={usageOpen}
        onOpenChange={setUsageOpenPersist}
        height={usageHeight}
        resize={usageResize}
        headerRef={usageHeaderRef}
        anyDragging={anyDragging}
      />
...
interface ProjectRowProps {
  ...
  homeDir: string;
  /** Start a new session scoped to this project (replaces Quick scripts). */
...
function ProjectRow({
  ...
  // Whether this workspace is a Git repository. `null` = not probed yet.
  // Probed lazily per row so the GitGraph button only shows for real repos.
  const [isGitWorkspace, setIsGitWorkspace] = useState<boolean | null>(null);
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/git/status?cwd=${encodeURIComponent(project.path)}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (cancelled) return;
        setIsGitWorkspace(data ? Boolean(data.isGitRepository) : false);
      })
      .catch(() => {
        if (!cancelled) setIsGitWorkspace(false);
      });
    return () => { cancelled = true; };
  }, [project.path]);
  ...
  return (
    <section className="sidebar-project" data-active={isActive ? "true" : "false"} style={{ marginBottom: 12 }}>
      ...
      >
        ...
        <Tooltip content={t("appShell.githubStatus")}>
          <button
            type="button"
        ...
        {/* Start a new session in this project (replaces Quick scripts). */}
