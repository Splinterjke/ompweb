...
export const ChatInput = memo(forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  /**
   * Empty new-session state: drop the asymmetric minimap-rail gutters so the
   * input box spans the full column, matching the hero cards and workspace
   * picker. Only applied on desktop (mobile padding path is untouched).
   */
  narrowColumn = false,
}: Props, ref) {
  ...
  // Invalidate the pending confirmation when delivery or navigation changes the queue.
  ...
  const modelSelectorDisabled = isStreaming || modelSwitching === true || (showModelsLoading && modelOptions.length === 0);
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (
        contextWrapRef.current && !contextWrapRef.current.contains(e.target as Node) &&
        !(sessionInfoWrapRef.current && sessionInfoWrapRef.current.contains(e.target as Node))
      ) {
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: CHAT_COLUMN_MAX_WIDTH, margin: "0 auto" }}>
              ...
              {plusMenuOpen && (
                <div
                  ...
                  style={{
                    ...
                    maxHeight: "min(340px, calc(100dvh - 32px))", overflowY: "auto",
                  }}
                >
        ...
        {/* Session Info — moved from the top bar (Interface & Behavior switch
            controls visibility; the Context gauge above is always shown). */}
        {sessionInfoButtonVisible && (sessionStats || contextUsage || modelCapacity || generationSpeed) && (
          <div
            ref={sessionInfoWrapRef}
            style={{ display: "flex", justifyContent: "center", position: "relative", marginTop: 4 }}
          >
            <SessionInfoButton
