  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", flexShrink: 0, height: open ? height : undefined, overflow: "hidden", transition: anyDragging ? "none" : "height var(--dur-med) var(--ease-out-warm)" }}>
      ...
      {open && (
        ...
        >
          {loadError && (
            <span style={{ fontSize: 11, color: "var(--status-error)" }}>{t("schedulers.loadError")}</span>
          )}
          {!loadError && !loadedRef.current && (
            <span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("schedulers.loading")}</span>
          )}
          {!loadError && list.length === 0 && (
            <>
              <span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("schedulers.empty")}</span>
              ...
              >
                ...
                {expanded && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "6px 9px 9px", borderTop: "1px solid var(--border)", maxHeight: detailMaxHeight, overflow: "hidden" }}>
                    <span style={{ fontSize: 10.5, fontFamily: "var(--font-mono)", color: "var(--text-dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={s.script}>
                      {s.script}
                    ...
                    {s.id === manualRunningId ? (
                      ...
                      <span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("schedulers.neverRun")}</span>
                    )}
                    ...
                    {s.id !== manualRunningId && s.runs.length > 1 && (
                      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        <span style={{ fontSize: 10, color: "var(--text-dim)", fontWeight: 600 }}>{t("schedulers.runs")}</span>
                        {s.runs.slice(1, 6).map((run) => (
                          <div key={run.id} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10.5, color: "var(--text-muted)" }}>
