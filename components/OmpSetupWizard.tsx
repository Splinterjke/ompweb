      ...
      <DialogContent ariaLabel={text("ompSetup.title", "Set up OMP runtime")} style={{ width: "min(92vw, 680px)", padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 18px", background: "linear-gradient(135deg, color-mix(in srgb, var(--accent) 16%, var(--bg-panel)), var(--bg-panel))", borderBottom: "1px solid var(--border)", position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--accent)", marginBottom: 8 }}>
            ...
            <span style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", fontFamily: "var(--font-mono)", fontWeight: 700, letterSpacing: ".08em" }}>LOCAL RUNTIME</span>
          </div>
          ...
          <p style={{ margin: 0, maxWidth: 550, color: "var(--text-muted)", fontSize: "calc(13px * var(--ui-font-scale-lg, 1))", lineHeight: 1.55 }}>
            {isInstalled
        ...
        <div style={{ padding: "18px 24px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderRadius: "var(--radius-control)", background: isInstalled ? "color-mix(in srgb, var(--status-success) 12%, var(--bg-subtle))" : "color-mix(in srgb, var(--status-warning) 12%, var(--bg-subtle))", border: `1px solid ${isInstalled ? "color-mix(in srgb, var(--status-success) 35%, var(--border))" : "color-mix(in srgb, var(--status-warning) 35%, var(--border))"}` }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontSize: "calc(12.5px * var(--ui-font-scale-lg, 1))", fontWeight: 650 }}>{isInstalled ? text("ompSetup.detected", "Runtime detected") : text("ompSetup.missing", "Runtime not detected")}</div>
              <div style={{ marginTop: 2, fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", color: "var(--text-muted)", fontFamily: "var(--font-mono)", overflowWrap: "anywhere" }}>{isInstalled ? `${diagnostics?.omp.version ?? "omp"} · ${diagnostics?.omp.path ?? ""}` : text("ompSetup.refreshHint", "Install it, reopen your terminal, then verify here.")}</div>
            </div>
            <button type="button" onClick={() => void refresh()} disabled={checking} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 8px", border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg)", color: "var(--text)", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", cursor: checking ? "wait" : "pointer" }}>
          ...
          {!isInstalled && <>
              ...
              {STEP_LABELS.map(({ id, label }, idx) => {
                ...
                return (
                  <div key={id} role="listitem" style={{ flex: 1, display: "flex", alignItems: "center", gap: 7, padding: "7px 9px", borderRadius: 8, background: state === "future" ? "var(--bg-subtle)" : state === "current" ? "color-mix(in srgb, var(--accent) 12%, var(--bg-panel))" : "color-mix(in srgb, var(--status-success) 10%, var(--bg-panel))", border: `1px solid ${state === "future" ? "var(--border)" : state === "current" ? "color-mix(in srgb, var(--accent) 45%, var(--border))" : "color-mix(in srgb, var(--status-success) 35%, var(--border))"}`, animation: state === "current" ? "ui-fade-in var(--dur-fast) var(--ease-out-warm) both" : undefined }}>
                    <span style={{ width: 18, height: 18, display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", fontSize: "calc(10.5px * var(--ui-font-scale-sm, 1))", fontWeight: 700, flexShrink: 0, color: state === "future" ? "var(--text-dim)" : state === "current" ? "var(--accent)" : "var(--status-success)", background: state === "future" ? "var(--bg)" : state === "current" ? "color-mix(in srgb, var(--accent) 14%, transparent)" : "color-mix(in srgb, var(--status-success) 12%, transparent)" }}>
                    ...
                    <span style={{ fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", fontWeight: state === "future" ? 500 : 650, color: state === "future" ? "var(--text-dim)" : "var(--text)" }}>{label}</span>
                  </div>
                );
              })}
            </div>
            <div>
              <div style={{ fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", fontWeight: 650, marginBottom: 8 }}>{text("ompSetup.choosePlatform", "Choose your platform")}</div>
              <div role="tablist" aria-label={text("ompSetup.choosePlatform", "Choose your platform")} style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {PLATFORMS.map(({ id, label }) => (
                  <button key={id} type="button" role="tab" aria-selected={selectedPlatform === id} onClick={() => setSelectedPlatform(id)} style={{ padding: "6px 10px", borderRadius: 7, border: "1px solid var(--border)", background: selectedPlatform === id ? "var(--bg-selected)" : "var(--bg)", color: selectedPlatform === id ? "var(--text)" : "var(--text-muted)", cursor: "pointer", fontSize: "calc(12px * var(--ui-font-scale-lg, 1))", fontWeight: selectedPlatform === id ? 650 : 500 }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div key={selectedPlatform} style={{ display: "flex", flexDirection: "column", gap: 8, animation: "ui-scale-in var(--dur-fast) var(--ease-out-warm) both" }}>
              ...
              {selectedPlatform !== "win32" && tools && !tools.curl && !tools.wget && (
                <div style={{ display: "flex", gap: 7, alignItems: "flex-start", padding: "8px 10px", borderRadius: 7, background: "color-mix(in srgb, var(--status-warning) 10%, var(--bg-subtle))", border: "1px solid color-mix(in srgb, var(--status-warning) 30%, var(--border))", color: "var(--text)", fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", lineHeight: 1.5 }}>
              ...
              {command.alternative && <div style={{ fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", color: "var(--text-muted)" }}>{text("ompSetup.alternative", "Alternative")}: <code style={{ fontFamily: "var(--font-mono)", color: "var(--text)" }}>{command.alternative}</code></div>}
            </div>
            ...
            <div style={{ padding: "12px", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", animation: "ui-fade-in var(--dur-med) var(--ease-out-warm) both" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: "calc(12.5px * var(--ui-font-scale-lg, 1))", fontWeight: 650, marginBottom: 2 }}><MonitorCog size={14} aria-hidden="true" /> {text("ompSetup.proxyTitle", "Behind a proxy?")}</div>
              <p style={{ margin: "0 0 4px", color: "var(--text-muted)", fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", lineHeight: 1.5 }}>{text("ompSetup.proxyBody", "Set the proxy before installing. OmpWeb passes it to OMP and its own network requests; automatic detection also checks common local proxy ports.")}</p>
              <NetworkProxyConfig />
            </div>
          </>}
          ...
          {selectedPlatform === "win32" && autoLaunchSupported && (
            <label style={{ display: "flex", gap: 9, alignItems: "flex-start", padding: "10px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", cursor: savingAutoLaunch ? "wait" : "pointer" }}>
              ...
              <span><span style={{ display: "block", fontSize: "calc(12.5px * var(--ui-font-scale-lg, 1))", fontWeight: 650 }}>{text("ompSetup.windowsStartup", "Open OmpWeb when I sign in")}</span><span style={{ display: "block", marginTop: 2, color: "var(--text-muted)", fontSize: "calc(11.5px * var(--ui-font-scale-sm, 1))", lineHeight: 1.45 }}>{text("ompSetup.windowsStartupBody", "The Windows installer also offers desktop and Start menu shortcuts. This option can be changed later.")}</span></span>
            </label>
          )}
        </div>
      </DialogContent>
