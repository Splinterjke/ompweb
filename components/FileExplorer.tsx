        ...
        <div role="group" inert={!open ? true : undefined} className={"accordion-flow " + (open ? "is-open" : "")}>
          <div className="accordion-flow-inner">
            ...
            {children.length === 0 && loaded && (
              <div style={{ paddingLeft: 8 + (depth + 1) * 14, fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)", height: 22, display: "flex", alignItems: "center" }}>
                {t("fileExplorer.emptyDir")}
              </div>
            )}
            {loadError && (
              <div role="alert" style={{ paddingLeft: 8 + (depth + 1) * 14, fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--status-error)", height: 22, display: "flex", alignItems: "center", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {loadError}
              </div>
            )}
          </div>
    ...
    <div style={{ minHeight: "100%" }}>
      ...
      {showUploadFeedback && (
        ...
        {uploadBusy && (
          <div role="status" aria-live="polite" aria-label={uploadPhase === "checking" ? t("fileExplorer.checkingFiles") : t("fileExplorer.uploadingPercent", { percent: uploadProgress })}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, minHeight: 14, color: "var(--text-muted)" }}>
              ...
              {uploadPhase === "uploading" && <span style={{ fontSize: "calc(10px * var(--ui-font-scale-sm, 1))" }}>{uploadProgress}%</span>}
            </div>
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, var(--status-warning) 55%, var(--border))", borderRadius: 4, background: "color-mix(in srgb, var(--status-warning) 9%, var(--bg-panel))" }}>
            <div style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
            ...
            {pendingConflict.nonReplaceable.length > 0 && (
              <div style={{ marginTop: 3, fontSize: "calc(10px * var(--ui-font-scale-sm, 1))", color: "var(--status-warning)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
                {t("fileExplorer.cannotReplace", { files: pendingConflict.nonReplaceable.join(", ") })}
              </div>
            )}
            <div style={{ display: "flex", gap: 5, marginTop: 7 }}>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "overwrite")} style={{ height: 24, minHeight: 24, padding: "0 8px", border: "1px solid var(--status-error)", borderRadius: "var(--radius-control)", background: "transparent", color: "var(--status-error)", cursor: "pointer", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>
                {t("fileExplorer.replace")}
              </button>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "skip")} style={{ height: 24, minHeight: 24, padding: "0 8px", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "var(--bg-panel)", color: "var(--text)", cursor: "pointer", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>
                {t("fileExplorer.skipExisting")}
              </button>
              <button type="button" onClick={() => setPendingConflict(null)} style={{ height: 24, minHeight: 24, padding: "0 8px", border: "none", borderRadius: "var(--radius-control)", background: "transparent", color: "var(--text-muted)", cursor: "pointer", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))" }}>
        ...
        {uploadError && (
          <div role="alert" style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", lineHeight: 1.35, color: "var(--status-error)" }}>
                ...
                <Tooltip content={uploadSummary.uploaded.length === 1 ? t("fileExplorer.addUploadedFile") : t("fileExplorer.addAllUploadedFiles")}>
                  <button
                    ...
                    style={{
                      ...
                      background: "var(--bg-panel)",
                      color: "var(--accent)",
                      cursor: "pointer", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", fontWeight: 600, whiteSpace: "nowrap",
      ...
      <div role="tree" aria-label={t("sessionSidebar.explorer")} aria-busy={loading} style={{ padding: "2px 4px" }}>
        ...
        {!loading && !error && roots.length === 0 && (
          <div style={{ padding: "8px 12px", fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)" }}>
