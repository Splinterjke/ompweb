...
test("panels start collapsed with live summary in their headers", () => {
  ...
  assert.match(html, /Tasks/);
  assert.match(html, /aria-label="0\/1"/);
...
test("chips show nested count and async marker, but not the agent source", () => {
  const html = renderToStaticMarkup(React.createElement(ComposerPanels, {
    todoPhases: [],
    subagents: [{
      id: "s1",
  ...
  assert.doesNotMatch(html, /data-subagent-metric="user"/);
