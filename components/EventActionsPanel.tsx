    ...
    <div style={{ flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", flexShrink: 0, height: open ? height : undefined, overflow: "hidden", transition: anyDragging ? "none" : "height var(--dur-med) var(--ease-out-warm)" }}>
        ...
        {open && (
          ...
          >
            {loadError && (
              <span style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--status-error)" }}>{t("chatActions.loadError")}</span>
            )}
            {!loadError && !loadedRef.current && (
              <span style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)" }}>{t("chatActions.loading")}</span>
            )}
            {!loadError && list.length === 0 && (
              <>
                <span style={{ fontSize: "calc(11px * var(--ui-font-scale-sm, 1))", color: "var(--text-dim)" }}>{t("chatActions.empty")}</span>
