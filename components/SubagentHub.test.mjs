...
test("mixed-status rows expose status badges and stay collapsed by default", () => {
  ...
  // The live (started) row has its pulsing status dot; the terminal rows use
  // lucide status icons (check/alert/ban) rather than a dot. Exactly one dot —
  // the running row — should be present, and it must be the pulsing variant.
  const dotCount = (expandedHtml.match(/class="live-status-dot/g) ?? []).length;
  assert.equal(dotCount, 1, "only the live (started) row renders a status dot");
...
test("hierarchy builder is deterministic and preserves parent ordering", () => {
  ...
  assert.deepEqual(
    ...
    ["root", "nested", "root-2", "unknown-child-2", "unknown-child"],
  );
